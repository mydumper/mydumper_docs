...
.. option:: --string-pk-planner

  Planner for string primary keys. Options: auto, metadata, recursive. Default: auto

.. option:: --string-pk-planner-timeout

  Maximum planner time per table in seconds. Default: 30

.. option:: --string-pk-planner-max-probes

  Maximum number of planner probes per table. Default: 64

.. option:: --string-pk-planner-max-prefixes

  Maximum number of root prefixes generated by the metadata-assisted planner. Default: 256

.. option:: --string-pk-planner-min-rows

  Minimum estimated rows before auto planner attempts metadata-assisted prefix planning. Default: 1000000

.. option:: --string-pk-planner-target-rows-per-prefix

  Target rows per prefix chunk for the metadata-assisted planner. 0 (default) derives it from table rows / --string-pk-planner-max-prefixes; a positive value is used directly and the planner deepens prefixes until reached. Bounded by --max-char-size (depth) and --string-pk-planner-max-prefixes (count). Default: 0
